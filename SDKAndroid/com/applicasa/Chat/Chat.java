...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	*/
	public Chat initWithCursor(Cursor cursor,String header,int level)
	{
		...
		if (columnIndex != LiCoreDBmanager.COLUMN_NOT_EXIST)
			...
			columnIndex = cursor.getColumnIndex(header + LiFieldChat.ChatRecipient.toString());
			if (columnIndex != LiCoreDBmanager.COLUMN_NOT_EXIST && level==0)
			{ 
				this.ChatRecipient = new User(cursor,LiFieldChat.ChatRecipient.toString(),level+1);
			}
			else{
				this.ChatRecipient = new User(cursor.getString(columnIndex));
			}
	...
	*/
	public String initWithObject(Chat item)
	{
		...
		this.ChatRecipient			= item.ChatRecipient;
...
public LiJSONObject dictionaryRepresentation(boolean withFK) throws LiErrorHandler {
	...
	try{
		...
		if (withFK)
			dictionary.put(LiFieldChat.ChatRecipient, ChatRecipient.dictionaryRepresentation(true));
		else
			dictionary.put(LiFieldChat.ChatRecipient, ChatRecipient.UserID);
		return dictionary;
		}
	...
	public static LiDbObject createDB() throws LiJSONException{
		...
		dbObject.put(LiFieldChat.ChatRecipient, LiCoreDBmanager.FOREIGN_KEY+"_User","0");
